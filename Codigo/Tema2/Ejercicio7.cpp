...
// Aplicar problema Prod Cons con monitores multiples
//
// -----------------------------------------------------------------------------
...
void MonitorPC::insertar(int dato){
    ...
    while (primera_libre == num_items) // si vemos que plibres, nuestro contador es igual al tamaño ya no quedan libres, los ponemos a espera
        libres.wait(guarda);
...
unsigned producir_dato(int valor) // produce mas de lo que pide error, jode el contador, hay que actualizarlo
{
   static int contador = valor * (num_items/tam_productoras) ;//
   ...
   cont_prod[contador] ++ ; // array compartido
   return contador++ ;
}
...
void funcion_hebra_productora(MonitorPC *monitor,int valor){
    while (true){
    //for (int i = 0; i < num_items; i++){
        int dato = producir_dato(valor);
